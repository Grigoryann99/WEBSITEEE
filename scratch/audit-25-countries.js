...
countryFiles.forEach((item, index) => {
  ...
  // Extract all objects from ts file
  ...
  if (images.length !== names.length) {
    console.log(`  ⚠️ WARNING: Found ${images.length} images for ${names.length} names!`);
  }
  ...
  for (let i = 0; i < names.length; i++) {
    ...
    if (img.startsWith("/images/")) {
      ...
      if (img.includes("wikimedia.org") || img.includes("wikipedia.org") || img.includes("unsplash.com")) {
        ...
        console.log(`  ⚠️ [#${i + 1}] "${name}": Unknown remote URL format -> ${img}`);
        cOK++;
        totalOK++;
      }
    } else {
